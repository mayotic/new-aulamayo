...
class CursosModel {
     ...
     */
     ...
     public function allowCertificate($id_curso, $id_usuario, $feedback_id, $item_id) {
       ...
       $feedback = $this->moodle_conn->table('mdl_feedback_completed')->where(['feedback' => $feedback_id, 'userid' => $moodle_user_id])->get();
       $item_grade = $this->moodle_conn->table('mdl_grade_grades')->where(['itemid' => $item_id, 'userid' => $moodle_user_id])->where('finalgrade', '>', 79)->get();

       if (!empty($feedback) and !empty($item_grade)) {
         return true;
       }
