...
class Pdox implements PdoxInterface
{
    ...
    /**
     * @var int Conection configuration
     */
    protected $config = [];
    ...
    public function table($table)
    {
       ...
       * @return $this
       */
      public function tableContext($table)
      {
          $this->table($table);
          $fields = $this->describeTable($table);
          $db_tables = $this->listTables($this->config['database']);
          var_dump($fields);
          var_dump($db_tables);
          return $this;
      }
     ...
     * @return $this|mixed
     */
    public function rawQuery($query, $type = null)
    {
        if (is_null($query)) {
            return null;
        }

        $rquery = $this->pdo->query($rquery);
        if (! $rquery) {
            $this->error = $this->pdo->errorInfo()[2];
            $this->error();
        }
    ...
    public function describeTable ($table) {
      return $this->query('DESCRIBE ' . $table)->fetchAll();
    }
    ...
    public function attachFilters($params) {
    ...
    public function attachOrder($params) {
        if(!empty($params['order'])){
          call_user_func_array([$this, 'orderBy'], $params['order']);
        }
    }

    public function attachPagination($params) {
        if(!empty($params['pagination'])){
          call_user_func_array([$this, 'pagination'], $params['pagination']);
        }
    }
