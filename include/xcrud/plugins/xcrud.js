	...
	request: function(container, data, success_callback) {
		...
		jQuery.ajax({
			...
			success: function(response) {
				// Mayo modal edit
				if ((Xcrud.current_task == 'edit' || Xcrud.current_task == 'create' || Xcrud.current_task == 'view') && !$(container).parent().hasClass('xcrud-nested-container')) {
					container = jQuery('#edit-modal .xcrud-ajax');
					jQuery(container).html(response);
					// jQuery('.xcrud > .xcrud-container > .xcrud-ajax input[name=key]').val($(container).find('input[name=key]').val());
					jQuery('#edit-modal').modal('show');
					$('#edit-modal').on('hidden.bs.modal', function () {
						jQuery('.xcrud > .xcrud-container > .xcrud-ajax input[name=key]').val($(container).find('input[name=key]').val());
						Xcrud.reload('.xcrud > .xcrud-container');

						// $.ajax({
						//     type: "post",
						//     url: Xcrud.config("url"),
						//     data: {xcrud: Xcrud.list_data(".xcrud  > .xcrud-container",{task:"list"})},
						//     success: function(response){
						//         $('.xcrud > .xcrud-container > .xcrud-ajax').html(response);
						// 				$('.xcrud-overlay').css('display', 'none');
						//     }
						// });
					...
					jQuery(container).html(response);
					if (!$(container).parent().hasClass('xcrud-nested-container')){
						jQuery('#edit-modal .modal-header .close').click();
					}
					jQuery('.xcrud > .xcrud-container > .xcrud-ajax input[name=key]').val($(container).find('input[name=key]').val());
...
jQuery(document).on("ready xcrudreinit", function() {
	...
	if ($(".xcrud").size()) {
		...
		$(".xcrud").on("click", ".xcrud-action", function() {
			...
			console.log(Xcrud);
			if (data.after == 'list'
					&& $(this).parents('.xcrud-nested-container').length == 0
					&& $(this).parents('#edit-modal').length == 1
					&& !Xcrud.validation_error) {
...
jQuery(document).on("xcrudafterrequest",function(event,container){
		if($(container).hasClass('modal-body')) {
				jQuery('.xcrud > .xcrud-container > .xcrud-ajax input[name=key]').val($(container).find('input[name=key]').val());
				// Xcrud.reload('.xcrud > .xcrud-container:not(.xcrud-nested-container)');
		}
});
