...
// Mayo modification for non The verification key is out ...
    ...
    protected function _set_column_names()
    {
        ...
        foreach ($this->columns as $key => $col)
        {
            ...
            elseif (isset($this->labels[$key]))
            {
                ...
                $this->columns_names[$key] = $this->labels[$key];
            }
    ...
    protected function _set_field_names()
    {
        foreach ($this->fields as $key => $field)
        {
            if (isset($this->labels[$key]))
                ...
                //     '');
                $this->fields_names[$key] = $this->labels[$key] . (isset($this->validation_required[$key]) ? '&#42;' :
                    '');
            else
    ...
    protected function _pagination_item($i, $curent, $limit)
    {
        $limit1 = $i * $limit - $limit;
        if ($i == $curent)
            return '<li class="' . $this->theme_config('pagination_active') . '"><span class="page-link xcrud-action">' . $i . '</span></li>';
        else
        {
            return '<li class="' . $this->theme_config('pagination_item') .
                '"><a href="javascript:;" class="page-link xcrud-action" data-start="' . $limit1 . '">' . $i . '</a></li>';
        }
    }
            ...
            if (isset($_SESSION['lists']['xcrud_session']) && $_SESSION['lists']['xcrud_session'])
            {
                ...
                { // workaround on some servers session duplication
                    ...
                    if ($time > $old_time + Xcrud_config::$autoclean_timeout) { // autocleaner
                        // Mayo modification for non "The verification key is out of date"
        ...
        'div', $tabs_head = 'ul', $tabs_row = 'li', $tabs_link = 'a', $tabs_content = 'div', $tabs_pane = 'div')
    {
        ...
        foreach ($this->fields_output as $key => $item)
        {
            ...
            // Mayo tweak
            $row_class = ((isset($this->field_attr[$key]['details_row']) and !empty($this->field_attr[$key]['details_row'])) ? $this->field_attr[$key]['details_row'] : $this->theme_config('details_row'));
            $cell_class = ((isset($this->field_attr[$key]['details_field_cell']) and !empty($this->field_attr[$key]['details_field_cell'])) ? $this->field_attr[$key]['details_field_cell'] : $this->theme_config('details_field_cell'));
            ...
            if (isset($this->fields[$item['name']]['tab']) && $this->fields[$item['name']]['tab'])
            {
                $tabs_out[$this->fields[$item['name']]['tab']][] = $this->open_tag($row, $row_class) . $this->open_tag($label, $this->
                    ...
                    close_tag($label) . $this->open_tag($field, $cell_class) . $item['field'] . $this->
            ...
            else
            {
                $raw_out[] = $this->open_tag($row, $row_class) . $this->open_tag($label, $this->theme_config('details_label_cell')) . $item['label'] .
                    $this->get_field_tooltip($item['name'], $mode) . $this->close_tag($label) . $this->open_tag($field, $cell_class) . $item['field'] . $this->close_tag($field) . $this->close_tag($row);
            }
        }
    ...
    protected function render_button($name = '', $task = '', $after = '', $class = '', $icon = '', $mode = '', $hide_text = false, $primary = '', $custom_action = false)
    {
        $out = '';
        ...
        if (!isset($this->hide_button[$name]))
        {
            ...
            $out .= $this->open_tag($tag, ($custom_action ? 'custom-action' : 'xcrud-action'));
            if ($icon && !$this->is_rtl)
            {
                $out .= $this->open_tag(array('tag' => 'i', 'class' => $icon)) . $this->close_tag('i') . ' ';
            }
            $out .= (!$hide_text ? $this->lang($name) : '');
            if ($icon && $this->is_rtl)
            {
                $out .= ' ' . $this->open_tag(array('tag' => 'i', 'class' => $icon)) . $this->close_tag('i');
            }
    ...
    protected function custom_button($name = '', $task = '', $after = '', $class = '', $icon = '', $new_window = true, $mode = '', $hide_text = false, $primary = '', $custom_action = false)
    {
        $new_window_class = '';
        if ($new_window) {
          $new_window_class = ' xcrud-in-new-window';
        }
        if (!isset($this->hide_button[$name]))
        {
            return $this->render_button($name, $task, $after, $class . $new_window_class, $icon, $mode, $hide_text, $primary, $custom_action);
        }
    }
